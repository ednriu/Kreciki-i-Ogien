...
{
        if (Paletka->Left<1) Paletka->Left = 320;
        Paletka->Left -= 10;
}
//---------------------------------------------------------------------------
...
{
        //paletka1
        if(Key == VK_LEFT) Timer_paletka_lewo->Enabled=true;
        if(Key == VK_RIGHT) Timer_paletka_prawo->Enabled=true;

        //paletka2
        if(Key == 'A') Timer_paletka2_lewo->Enabled=true;
        if(Key == 'D') Timer_paletka2_prawo->Enabled=true;
...
{
        //paletka1
        if(Key == VK_LEFT) Timer_paletka_lewo->Enabled=false;
        if(Key == VK_RIGHT) Timer_paletka_prawo->Enabled=false;

        //paletka2
        if(Key == 'A') Timer_paletka2_lewo->Enabled=false;
        if(Key == 'D') Timer_paletka2_prawo->Enabled=false;
...
void __fastcall TKreciki::Timer_pilkaTimer(TObject *Sender)
{
        ...
        if (Paletka2->Visible==false)
        {
                ...
                if (Pilka->Top>520 && Kolory->Brush->Color==clGreen) y=-y;

                //odbijanie od paletki 1
                if(Pilka->Top>Paletka->Top-Pilka->Height && Pilka->Left>Paletka->Left-Pilka->Width/2 && Pilka->Left<Paletka->Left+Paletka->Width)
                {
                        if (y>0)
                        {
                           y=-y;
                ...
                } else if(Pilka->Top>Paletka->Top+(Pilka->Height/2))    //Skucha
                {
                        Pilka->Visible = false;
        ...
        if (Paletka2->Visible==true)
        {
                ...
                if (Pilka->Top>520 && Kolory->Brush->Color==clGreen) y=-y;
                if (Pilka->Top<2 && Kolory->Brush->Color==clRed) y=-y;


                //Skucha
                if((Pilka->Top>Paletka->Top+Pilka->Height/2) || (Pilka->Top<Paletka2->Top-Pilka->Height))
                {
                        Pilka->Visible = false;
                        Timer_pilka->Enabled = false;
                        Timer_krety ->Enabled = false;
                ...
                if(Pilka->Top<Paletka2->Top+Paletka2->Height && Pilka->Left>Paletka2->Left-Pilka->Width/2 && Pilka->Left+Pilka->Width/2<Paletka2->Left+Paletka2->Width)
                {
                        if (y<0)
                        {
                        y=-y;
...
void __fastcall TKreciki::StartClick(TObject *Sender)
{
        ...
        if ((Start->Caption=="Start") || (Start->Caption=="Nowa Gra"))
        {
                ...
                Pilka->Left = random(10, 350);
                Pilka->Top = random(150, 400);
        };
...
{
        if (Paletka->Left>320) Paletka->Left = 1;
        Paletka->Left+=10;
}
//---------------------------------------------------------------------------
...
void __fastcall TKreciki::Timer_kretyTimer(TObject *Sender)
{
        ...
        {
                Krecik->Left+=a;
                if (Krecik->Left<=5) a=-a;
                if (Krecik->Left>=375) a=-a;
        };
        ...
        {
                Krecik4->Left+=a;
                if (Krecik4->Left<=5) a=-a;
                if (Krecik4->Left>=375) a=-a;
        };

        if (Krecik7->Visible)
        {
                Krecik7->Left+=a;
                if (Krecik7->Left<=5) a=-a;
                if (Krecik7->Left>=375) a=-a;
        };

        if (Krecik10->Visible)
        {
                Krecik10->Left+=a;
                if (Krecik10->Left<=5) a=-a;
                if (Krecik10->Left>=375) a=-a;
        };
        ...
        {
                Krecik2->Left+=b;
                if (Krecik2->Left<=5) b=-b;
                if (Krecik2->Left>=375) b=-b;
        };

        if (Krecik5->Visible)
        {
                Krecik5->Left+=b;
                if (Krecik5->Left<=5) b=-b;
                if (Krecik5->Left>=375) b=-b;
        };

        if (Krecik8->Visible)
        {
                Krecik8->Left+=b;
                if (Krecik8->Left<=5) b=-b;
                if (Krecik8->Left>=375) b=-b;
        };

        if (Krecik11->Visible)
        {
                Krecik11->Left+=b;
                if (Krecik11->Left<=5) b=-b;
                if (Krecik11->Left>=375) b=-b;
        };
        ...
        {
                Krecik3->Left+=c;
                if (Krecik3->Left<=5) c=-c;
                if (Krecik3->Left>=375) c=-c;
        };

                if (Krecik6->Visible)
        {
                Krecik6->Left+=c;
                if (Krecik6->Left<=5) c=-c;
                if (Krecik6->Left>=375) c=-c;
        };

                if (Krecik9->Visible)
        {
                Krecik9->Left+=c;
                if (Krecik9->Left<=5) c=-c;
                if (Krecik9->Left>=375) c=-c;
        };

                if (Krecik12->Visible)
        {
                Krecik12->Left+=c;
                if (Krecik12->Left<=5) c=-c;
                if (Krecik12->Left>=375) c=-c;
        };
...
{
        if (Paletka2->Left<1) Paletka2->Left = 320;
        Paletka2->Left -= 10;        
}
//---------------------------------------------------------------------------
...
{
        if (Paletka2->Left>320) Paletka2->Left = 1;
        Paletka2->Left+=10;        
}
//---------------------------------------------------------------------------
...
void __fastcall TKreciki::TimerOgienTimer(TObject *Sender)
{
        int losowa1, losowa2;
        bool kolizja = false;
        losowa1 =  random(100,550-Ogien->Height-100);
        losowa2 =  random(50,280);
        Ogien->Enabled  =true;
        Ogien->Visible = true;
...
void __fastcall TKreciki::TimerKoloryTimer(TObject *Sender)
{
        ...
        if (numerKoloru>=1 && numerKoloru<=4) if((y>0 || Pilka->Top>50) && (y<0 || Pilka->Top<450)) Kolory->Brush->Color = clRed;
        if (numerKoloru>=5 && numerKoloru<=8) if((y>0 || Pilka->Top>50) && (y<0 || Pilka->Top<450)) Kolory->Brush->Color = clGreen;
        if (numerKoloru>=9 && numerKoloru<=12) if((y>0 || Pilka->Top>50) && (y<0 || Pilka->Top<450)) Kolory->Brush->Color = clGray;
        if (numerKoloru>=13 && numerKoloru<=16) if((y>0 || Pilka->Top>50) && (y<0 || Pilka->Top<450)) Kolory->Brush->Color = clAqua;
        if (numerKoloru>=17 && numerKoloru<=20) if((y>0 || Pilka->Top>50) && (y<0 || Pilka->Top<450)) Kolory->Brush->Color = clFuchsia;
        if (numerKoloru>=21 && numerKoloru<=24) if((y>0 || Pilka->Top>50) && (y<0 || Pilka->Top<450)) Kolory->Brush->Color = clYellow;
}
//---------------------------------------------------------------------------
